...
class Database {
	...
	public function getColumns($table) {
		...
		$schema = $this->resultset();
		$columns = array();
		for ($i = 0; $i < count($schema); $i++) {
			$columns[] = $schema[$i]['Field'];
		}
		return $columns;
	}
    ...
    public function update($array, $table, $id, $idkey = 'id') {
        ...
        $this->query('UPDATE ' . $table . ' SET ' . implode(', ', $fields) . ' WHERE ' . $idkey . ' = :' . $idkey);
        foreach($array as $key => $value) {
            $this->bind(':' . $key, $value);
        }
        $this->bind(':' . $idkey, $id);
    ...
    public function select($array, $table, $where = array()) {
        ...
        if (count($where)) {
            ...
            $this->query('SELECT ' . implode(', ', $array) . ' FROM ' . $table . ' WHERE ' . implode(' AND ', $whereFields));
        } else {
            $this->query('SELECT ' . implode(', ', $array) . ' FROM ' . $table);
        }
